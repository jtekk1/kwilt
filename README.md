...
- **Resizing a tiled window** (e.g. `Meta+Right-drag` on Plasma defaults) adjusts the layout: horizontal edge drags that move the master-vs-non-master boundary update `MasterWidth`; vertical edge drags update per-column row-height ratios (centerTile side columns and leftTile / rightTile 2-column mode). Drags between two non-master columns (leftTile / rightTile 2-col inner ↔ outer) currently snap back — inter-column split state is a follow-up. Both writes are in-memory only and reset on script reload; persistence is a shared write-path follow-up.
