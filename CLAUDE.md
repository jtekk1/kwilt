...
**Runtime model.** This script runs *inside the KWin process* on a QJSEngine. Globals like `workspace`, `KWin`, `print`, `registerShortcut`, and `callDBus` are injected by KWin — they are not standard JS. There is no `console`, no `require`/`import`, no `setTimeout` you should rely on, no `child_process`/`os.system`/shell. Signals are connected with `.connect(fn)`. Window objects are live Qt objects; properties like `w.frameGeometry`, `w.minimized`, `w.output`, `w.desktops` mutate the WM directly when set. **New KWin globals you introduce in code must also be added to `eslint.config.js`'s `globals` map**, else lint fails with `no-undef`.

**Spawning processes.** KWin scripts cannot exec directly — `callDBus` is the only escape hatch. `spawnApp(cmd)` uses it to ask `org.freedesktop.systemd1.Manager.StartTransientUnit` to start `/bin/sh -c <cmd>` as a unique transient user unit. The shell wrapper is intentional so shortcuts can use `$HOME`, pipes, and quoting the way users expect from a config file. Transient-unit names include `Date.now()` to avoid collisions when the same shortcut fires rapidly. If `callDBus` is missing (some KWin builds), `spawnApp` no-ops with a log line — launcher shortcuts register but do nothing.

**Shortcut data flow.** Layout / focus / swap shortcuts call into the tiling functions directly. App launchers are data-driven from the `APP_SHORTCUTS` table at the top of `main.js` — each entry yields one `registerShortcut` whose callback closes over `s.cmd` and hands it to `spawnApp`. Add a new launcher by appending one entry to `APP_SHORTCUTS`; no other changes needed.
